...
function loadIntroText() {
  ...
  loader.load('fonts/unione.json', function (font) {
    ...
    const titleGeo = new TextGeometry('KULASEKARA MUTHU', {
      font: font,
      size: 0.07,
      height: 0.02,
    });
  ...
  loader.load('fonts/helvatica.json', function (font) {
    ...
    const subTitleGeo = new TextGeometry(
      'Web Developer /problem Solver/ Content Creator',
      {
        font: font,
        size: 0.020,
        height: 0,
...
function cameraToAbout() {
  ...
  // Step 1: Move to about position & rotation
  ...
  // Step 3: After book open, zoom *straight forward*
  tl.add(() => {
    ...
    // Decide zoom distance
    const zoomDistance = 0.1;

    // New position = current + forward * distance
    const newPos = camera.position.clone().add(forward.multiplyScalar(zoomDistance));

    // Animate move straight in
    gsap.to(camera.position, {
      x: newPos.x,
      y: newPos.y,
      z: newPos.z,
